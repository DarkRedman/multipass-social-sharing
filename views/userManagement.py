from flask import current_app, Blueprint, render_template, request, make_response, redirect
import database
...
def route_to_create_account():
    if request.method == 'GET':
        return render_template('createUser.html', baseurl=current_app.config['BASEURL'])
    if request.method == 'POST':
        ...
        if not database.user_exist(email):
            ...
            response = make_response(redirect(current_app.config['BASEURL'] + 'login'))
